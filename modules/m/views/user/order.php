...
<div class="order_box mg-t20">
	...
	<?php if( $_item['status'] == -8 ):?>
        <div class="op_box border-top">
            <a style="display: inline-block;" class="button close" data="<?=$_item['id'];?>" href="<?=UrlService::buildNull();?>">取消订单</a>
            <a style="display: inline-block;" class="button"  href="<?=$_item["pay_url"];?>">微信支付</a>
        </div>
	<?php elseif( $_item['status'] == 1 && $_item['express_status'] == -6):?>
            <div class="op_box border-top">
                <a style="display: inline-block;" data="<?=$_item['id'];?>"  href="<?=UrlService::buildNull();?>"  class="button confirm_express">确认收货</a>
            </div>
	<?php endif;?>
</div>
